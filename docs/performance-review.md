...
| Concurrent resource creation, 8 cold acquisitions / 20 ms factories | 244.60 | 29.12 | 88.1% |
...
Resource creation reserves capacity before starting factories and assigns each completed resource to the oldest remaining waiter. The startup benchmark observed eight simultaneous creations instead of one. Tests use deferred factories completed in reverse order to check FIFO assignments, capacity limits, reuse, and cancellation followed by drain. Factories are now allowed to overlap up to `maxSize`; factory implementations should support that concurrency. Raw samples: [pool-startup-after.json](validation/pool-startup-after.json).

A separate comparison of the existing 50,000-operation reuse benchmarks found sequential reuse at 100.30 ms before / 98.02 ms after, and contended reuse at 201.62 ms before / 202.82 ms after. These small differences do not demonstrate a meaningful regression or gain. Raw samples: [pool-reuse-comparison.json](validation/pool-reuse-comparison.json).
